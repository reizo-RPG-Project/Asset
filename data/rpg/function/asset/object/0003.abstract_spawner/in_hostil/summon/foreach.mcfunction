...
# 先頭取得
data modify storage reizo_mcfunc_engin:context this.SpawnMob.ID set from storage reizo_mcfunc_engin:context this.SpawnMob[0]

# 召喚
    data modify storage reizo_mcfunc_engin:lib In.For.LoopCount set from storage reizo_mcfunc_engin:context this.SpawnCount
...
# 先頭が残るなら再帰
execute if data storage reizo_mcfunc_engin:context this.SpawnMob[0] run function rpg:asset/object/0003.abstract_spawner/in_hostil/summon/foreach