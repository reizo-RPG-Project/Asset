...
#> 見つかったら詳細を表示
    ...
    # 基本データ
    # 名前
        data modify storage reizo_mcfunc_engin:context this.Lore.Name set value ["",{"text":"\uE010",font:"icon/_"},{"text":"名前: "},{"entity":"@n[tag=RPG.Item.9000.Target]",nbt:"CustomName"},"\n"]
        execute if data storage reizo_mcfunc_engin:context this.TargetData.IconID run data modify storage reizo_mcfunc_engin:context this.Lore.Name insert 3 value {storage:"reizo_mcfunc_engin:context",nbt:"this.TargetData.IconID",font:"icon/enemies"}
    # 説明文
