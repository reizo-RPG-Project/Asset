...
# ランダムに決める
    ...
    # 取得した長さからランダムに選ぶ
    execute if data storage reizo_mcfunc_engin:context this.SetItem.SetGetData.Length{Value:1} run data modify storage reizo_mcfunc_engin:context this.SetItem.data set from storage reizo_mcfunc_engin:context data.Field.Pools[0]
